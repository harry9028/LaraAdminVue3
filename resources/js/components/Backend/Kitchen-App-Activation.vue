...
export default {
  ...
  methods: {
    ...
    getAppActivationList() {
      this.isLoad = true;
      axios.get("api/kitchen-app-activation").then((response) => {
        this.isLoad = false;
    ...
    getResults(page = 1) {
      ...
      axios
        .get(
          "api/kitchen-app-activation?q=" + this.search + "&page=" + this.page
        )
        .then((response) => {
          this.isLoad = false;
          this.AppActivationList = response.data;
    ...
    updateUser() {
      this.form
        .post("api/kitchen-app-activation/" + this.form.id)
        .then(() => {
          $("#newModal").modal("hide");
  ...
  mounted() {
    ...
    this.access_level =
      window.gate.user.access_level.pos_offline_activation.kitchen_app;
  },
};
</script>
